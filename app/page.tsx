...
function WorkCard({ work, onOpen }: { work: Work; onOpen: (work: Work) => void }) {
  ...
  return <button className="work-card" onClick={() => onOpen(work)} aria-label={`查看《${work.title}》`}>
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="map-section" id="map"><div className="section-heading"><div><p className="section-kicker">FUTURE MAP / 未来地图</p><h2>你今天想理解<br />哪一种未来？</h2></div><p>从一个命题进入，再跨过媒介的边界。每一张卡片都说明：它提出了什么问题，以及为什么值得成为 City2049 的参考。</p></div>
      <div className="topic-strip" role="tablist" aria-label="未来主题">{topics.map((item) => <button key={item.id} className={topic === item.id ? "active" : ""} onClick={() => setTopic(item.id)} role="tab" aria-selected={topic === item.id}><span>{item.symbol}</span>{item.label}</button>)}</div>
      <div className="filter-row"><div className="medium-filter" aria-label="媒介筛选">{(["全部", "书", "电影 / 剧集", "游戏"] as const).map((item) => <button key={item} className={medium === item ? "active" : ""} onClick={() => setMedium(item)}>{item}</button>)}</div><label className="search-box"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索作品、作者或问题" aria-label="搜索作品" /></label><span className="result-count">{String(filtered.length).padStart(2, "0")} RESULTS</span></div>
      <div className="work-grid">{filtered.map((work) => <WorkCard key={work.id} work={work} onOpen={setSelected} />)}</div>{!filtered.length && <div className="empty-state"><b>这一格暂时还是空白。</b><span>试试换一个主题或清除搜索词。</span></div>}
    </section>
